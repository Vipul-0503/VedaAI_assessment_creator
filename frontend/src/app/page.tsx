...
import { Search, Bell, ChevronDown, SlidersHorizontal, MoreVertical, Plus, FileCheck2, Loader2, LayoutGrid } from "lucide-react";
    ...
    <div className="flex flex-col space-y-6">
      ...
      {loading ? (
        ...
        <div className="bg-gray-50 border border-dashed border-gray-200 rounded-3xl flex flex-col items-center justify-center text-center p-16 max-w-2xl mx-auto mt-6">
          ...
          <Link href="/create">
            <button className="bg-[#1a1a1a] hover:bg-black text-white text-sm font-medium px-5 py-3 rounded-xl flex items-center gap-2 shadow-sm transition-all mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredAssessments.map((assessment) => (
            <div key={assessment._id} className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between h-44 group relative">
              <div>
                <div className="flex items-start justify-between">
                  <Link href={`/assessment/${assessment._id}`} className="hover:underline cursor-pointer">
                    <h3 className="font-bold text-gray-900 text-base leading-snug pr-6 group-hover:text-orange-600 transition-colors">{assessment.title}</h3>
                  </Link>
                  <button className="text-gray-400 hover:text-gray-900 p-1 absolute top-4 right-4 rounded-lg"><MoreVertical className="h-4 w-4" /></button>
                </div>
                <p className="text-xs text-gray-400 mt-1 font-medium truncate">Topic: {assessment.topic}</p>
              </div>
              <div className="border-t border-gray-100 pt-3 flex items-center justify-between text-[11px] font-semibold text-gray-500">
                <div>Assigned on : <span className="text-gray-800">{new Date(assessment.createdAt).toLocaleDateString()}</span></div>
                <span className={`px-2 py-0.5 rounded-full text-[10px] uppercase tracking-wider ${assessment.status === 'completed' ? 'bg-green-100 text-green-700' : assessment.status === 'failed' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{assessment.status}</span>
              </div>
            </div>
          ))}
        </div>
